...
export class CanvasTable {
  ...
  private static readonly TRANSPARENT_COLOR = 'rgba(255,255,255,0)'
  ...
  constructor(canvas: HTMLCanvasElement, config: CTConfig) {
    this.canvas_ = canvas
  ...
  private calculateColumnTextWidths(): number[] {
    ...
    for (const rowIndex in data) {
      ...
      for (const cellIndex in row) {
        const [cellValue] = (row[cellIndex] || '').toString().split('\n')
  ...
  private calculateColumnWidths(): void {
    ...
    if (columnWidthTotal > tableWidth) {
      ...
      this.computedOuterWidths = columnTextWidths.map(
        width => width + (difference * width) / totalColumnWidths + this.horizontalTotalPadding
      )
    }
  }
  ...
  private generateTitle(title: CTTitle): void {
    ...
    const titleLines = title.text.toString().split('\n')
  ...
  private async generateRows() {
    ...
    for (let rowIndex = 0; rowIndex < data.length; rowIndex++) {
      ...
      for (const cellIndex in columnOuterWidths) {
        ...
        const columnOptions = columns && columns[cellIndex].options ? columns[cellIndex].options : {}
        ...
        if (outerWidth > computedOuterWidth) {
          ...
          if (isFat()) {
            ...
            cellValue = cellValue.length > minCharWidth ? `${cellValue}${CanvasTable.ELLIPSIS}` : `${cellValue}.`
          }
        }
        ...
        if (options) {
          ...
          let fontWeight = options.fontWeight || cell.fontWeight
          let fontSize = options.fontSize || cell.fontSize
          let fontFamily = options.fontFamily || cell.fontFamily
          let fontColor = options.color || cell.color
